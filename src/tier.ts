 ...
 */
export const TIER_WEIGHT: Record<Tier, number> = { 1: 1.35, 2: 1.15, 3: 1, 4: 0.7, 5: 0.5 };
 ...
 */
export function flightOrder(division: string | null | undefined): number {
...
function base(d: string): number {
  if (/best of the/.test(d)) return 0;
  if (/\bsuper\s+black\b/.test(d)) return 1;
  if (/\bsuper\s+white\b|\bsuper\b/.test(d)) return 2;
  // Red is a local cup's top flight (Eastside FC Cup, Starfire), where Gold is another's.
  if (/premier|elite|\bgold\b|\bred\b|championship|\b(div(ision)?|flight|tier)\.?\s*(1|i|a)\b|\b1st\b|open/.test(d)) return 3;
  if (/\bblack\b|silver|select|classic|\b(div(ision)?|flight|tier)\.?\s*(2|ii|b)\b|\b2nd\b/.test(d)) return 4;
  if (/\bwhite\b|\b(div(ision)?|flight|tier)\.?\s*(3|iii|c)\b|\b3rd\b/.test(d)) return 5;
  if (/\bblue\b|bronze|copper|\bgreen\b|\b(div(ision)?|flight|tier)\.?\s*([4-9]|iv|d)\b|\b[4-9]th\b/.test(d)) return 6;
  if (/\bgr[ae]y\b|\brec\b|recreational/.test(d)) return 7;
...
export function flightStep(division: string | null | undefined): 0 | 1 | 2 {
  const d = (division ?? "").toLowerCase();
  if (!d) return 0;
  // "Best of the Best", "Premier", "Elite", "Gold", "Championship", "Div 1", "Flight 1"
  if (/best of the|premier|elite|\bgold\b|championship|\b(div(ision)?|flight|tier)\.?\s*(1|i)\b|\b1st\b|open/.test(d)) return 0;
  ...
  if (/\bsuper\b|\bblack\b|\bwhite\b|select|silver|classic|\b(div(ision)?|flight|tier)\.?\s*(2|ii)\b|\b2nd\b/.test(d)) return 1;
  // "Blue", "Grey", "Bronze", "Copper", "Rec", "Div 3"
  if (/\bblue\b|\bgr[ae]y\b|bronze|copper|\brec\b|recreational|\b(div(ision)?|flight|tier)\.?\s*([3-9]|iii|iv)\b|\b[3-9](rd|th)\b/.test(d)) return 2;
 ...
 */
export type FlightLevel = { tier: Tier; step: number };

export function flightLevel(eventTier: number | null | undefined, division: string | null | undefined, override: number | null | undefined = null, step: number | null = null): FlightLevel | null {
  if (override && override >= 1 && override <= 5) return { tier: override as Tier, step: 0 };
  if (!eventTier || eventTier < 1 || eventTier > 5) return null;
  return { tier: eventTier as Tier, step: Math.max(0, step ?? flightStep(division)) };
}

/** The tier a flight plays at — the tournament's, or the division's own word. Null where the event is untiered. */
export function divisionTier(eventTier: number | null | undefined, division: string | null | undefined, override: number | null | undefined = null, step: number | null = null): Tier | null {
...
function flightShare(step: number, per: number, floor: number): number {
...
export type FlightRead = { step: number; by: "teams" | "name"; strength: number | null; teams: number };

/** "girls-13" — the age and gender a flight's name says, so flights are only compared with their own. */
export function flightGroup(name: string | null | undefined): string {
...
export function orderFlights(flights: { id: string; name: string | null; priors: number[] }[]): Map<string, FlightRead> {
  const out = new Map<string, FlightRead>();
  ...
  for (const f of flights) {
    ...
    if (/championship/i.test(f.name ?? "")) { out.set(f.id, { step: 0, by: "name", strength: null, teams: 0 }); continue; }
  ...
  for (const group of groups.values()) {
    ...
    const byName = (f: (typeof flights)[number]): FlightRead => ({ step: ranks.indexOf(flightOrder(f.name)), by: "name", strength: f.priors.length ? median(f.priors) : null, teams: f.priors.length });
    const known = group.filter((f) => f.priors.length >= MIN_FLIGHT_TEAMS);
    if (known.length < 2) {
      for (const f of group) out.set(f.id, byName(f));
      continue;
    }
    const ordered = known
    ...
    for (const f of group) if (!out.has(f.id)) out.set(f.id, byName(f));
  }
  return out;
}

/** How much a result counts, by what it was played in: the tier's weight, less a tenth per flight below the top. */
export function weightOf(kind: string, level: FlightLevel | null): number {
  if (kind === "league") return 1;
  if (kind === "tournament") return level ? TIER_WEIGHT[level.tier] * flightShare(level.step, FLIGHT_WEIGHT_STEP, TIER_WEIGHT[5] / TIER_WEIGHT[level.tier]) : UNTIERED_TOURNAMENT;
  return 0.6;
}

export const TIER_LABELS: Record<Tier, string> = { 1: "Tier 1 — national", 2: "Tier 2 — regional", 3: "Tier 3 — premier", 4: "Tier 4 — select", 5: "Tier 5 — rec" };
 ...
 */
export const CHAMPION_BONUS: Record<Tier, number> = { 1: 240, 2: 150, 3: 80, 4: 40, 5: 20 };
...
export function honourBonus(place: "champion" | "runner-up", level: FlightLevel | null, division: string | null | undefined = null): number {
  const full = level
