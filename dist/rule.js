...
export function carriedPrior(leaguePrior, last) {
    if (!last || last.games < MIN_GAMES)
        return leaguePrior;
    return leaguePrior + CARRY * (last.rating - leaguePrior);
}
export const SELECT = { label: "select", prior: 1400 };
/**
 ...
 */
export const TIER = { one: 1800, two: 1740, three: 1660, four: 1560, five: 1490, six: 1440 };
export function levelOf(text, age = null) {
    const t = text.toLowerCase();
    const n = (re) => { const m = t.match(re); return m ? Number(m[1]) : null; };
    const young = age !== null && age <= 12;
    if (/pre-?ecnl/.test(t))
        return /flight\s*ii/.test(t) ? { label: "Pre-ECNL Flight II", prior: 1770 } : { label: "Pre-ECNL", prior: TIER.one };
    if (/ecnl\s*(rl|regional)/.test(t))
        return { label: "ECNL RL", prior: TIER.three };
    if (/\becnl\b/.test(t))
        return { label: "ECNL", prior: TIER.one };
    if (/aspire/.test(t))
        return { label: "GA Aspire", prior: TIER.four };
    if (/girls academy|\bga\b|pre-?ga/.test(t))
        return { label: "GA", prior: young ? TIER.two : TIER.three };
    if (/elite academy|\bea\b|pre-?ea/.test(t))
        return { label: "EA", prior: young ? TIER.two : TIER.three };
    if (/mls\s*next/.test(t))
        return { label: "MLS Next", prior: TIER.two };
    if (/\bwpl\b|washington premier league/.test(t)) {
        ...
        if (/\bdev\b/.test(t)) {
            const d = n(/div\.?\s*(\d)/) ?? 3;
            return { label: `WPL Dev ${d}`, prior: d === 1 ? TIER.two : d === 2 ? TIER.four : d === 3 ? TIER.five : d === 4 ? TIER.six : 1400 };
        }
        if (/premier/.test(t))
            return { label: "WPL Premier", prior: TIER.four };
        if (/classic/.test(t))
            return { label: "WPL Classic", prior: TIER.five };
        if (/copa/.test(t))
            return { label: "WPL Copa", prior: 1400 };
    ...
    if (/\brcl\b|regional club league/.test(t)) {
        ...
        if (age !== null && age <= 10)
            return { label: `RCL ${d}`, prior: d === 1 ? TIER.one : d === 2 ? TIER.three : TIER.four };
 ...
 */
export function leagueWeightOf(level) {
...
export function bestLevel(candidates) {
    const real = candidates.filter((l) => !!l).sort((a, b) => b.prior - a.prior);
    return real[0] ?? SELECT;
}
/** How much older the home side is than the away side, in years; 0 when either has no birth year. */
export function ageGapOf(homeYear, awayYear) {
    if (homeYear === null || awayYear === null)
        return 0;
 ...
 */
export function replay(games, prior, yearOf = () => null) {
    const out = new Map();
    const credited = new Map();
    const get = (id) => out.get(id) ?? { rating: prior(id), games: 0 };
...
export function cohortOf(team) {
    const years = (team.birthYears ?? []).filter((y) => y > 2000 && y < 2030);
    if (!years.length)
        return null;
    if (team.gender !== "boys" && team.gender !== "girls")
        return null;
 ...
 */
export function ageGroupOf(cohort, seasonStart) {
